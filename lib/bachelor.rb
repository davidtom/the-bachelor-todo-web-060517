  ...
  data.each { |season_number, contestants|
    if season_number == season
      contestants.each { |contestant|
        winner_name = contestant["name"] if contestant["status"] == "Winner"
      }
    end
  }
  winner_name.split[0]
...
def get_occupation(data, hometown)
  matching_occupation = String.new
  data.each { |season_number, contestants|
    contestants.each { |contestant|
      if contestant["hometown"] == hometown
        matching_occupation = contestant["occupation"]
        break
      end
    }
  }
...
def get_average_age_for_season(data, season)
  ...
  data.each { |season_number, contestants|
    if season_number == season
      contestants.each { |contestant|
        running_sum += contestant["age"].to_f
        running_n += 1
      }
    end
  }
  average_age = running_sum/running_n
  average_age.round
end
